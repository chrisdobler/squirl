...
export async function syncInferredTaskEvents(options: {
  ...
  for (const task of options.snapshot.tasks) {
    ...
    if (index < 0) {
      ...
      const eventId = await options.client.createTaskEvent(options.calendarId, { taskId: task.id, title: task.title, startAt, endAt });
      state.entries.push({ taskId: task.id, title: task.title, calendarId: options.calendarId, eventId, startAt, endAt, lastSeenAt: nowAt, lastActiveAt: task.lastActiveAt, status: 'active' });
    ...
    const endAt = eventEnd(entry.startAt, now, options.activeHorizonMs);
    await options.client.updateTaskEvent(entry.calendarId, entry.eventId, { taskId: task.id, title: task.title, startAt: entry.startAt, endAt });
    state.entries[index] = { ...entry, taskId: task.id, title: task.title, endAt, lastSeenAt: nowAt, lastActiveAt: task.lastActiveAt };
    matched.add(index);
    options.save(state);
  }
  ...
  for (let index = 0; index < state.entries.length; index++) {
    ...
    await options.client.updateTaskEvent(entry.calendarId, entry.eventId, { taskId: entry.taskId, title: entry.title, startAt: entry.startAt, endAt });
